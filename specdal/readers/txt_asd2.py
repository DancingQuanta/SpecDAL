...
def read_txt_asd2(filepath, read_data=True, read_metadata=True, verbose=False):
    """
    Read text asd file from FS4 for data and metadata
    ...
    with open(abspath(expanduser(filepath)), 'rU') as f:
        ...
        vnir_integration_time = next(reader)[0]
        # read wavelength info for vnir
        wave_config_one = next(reader)[0]
        assert("VNIR channel 1 wavelength" in wave_config_one)
        wave_config_one = wave_config_one.split(' ')
        wavestart = wave_config_one[5]
        wavestep = wave_config_one[9]
        ...
        # SWIR gains
        assert("SWIR1 gain was" in next(reader)[0])
        assert("SWIR2 gain was" in next(reader)[0])
        # Joins between detectors
        assert("Join between VNIR and SWIR1 was" in next(reader)[0])
        assert("Join between SWIR1 and SWIR2 was" in next(reader)[0])
        # Dark current
        ...
        if read_metadata:
            ...
            splice2 = None
            # integration time
            integration_time = vnir_integration_time.split(' ')[-1]
            # gps info
